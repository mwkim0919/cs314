...
// added: added for arm
var leftupperarmMatrix = new THREE.Matrix4().set(
...
var leftforearmMatrix = new THREE.Matrix4().set(
...
// added: left arm rotation matrix
  if (Math.cos(t/2) > 0 && Math.sin(t/2) > 0) {
    var rotMatrixX = new THREE.Matrix4().set(
      1,0,0,0,
    ...
    var rotMatrixX = new THREE.Matrix4().set(
      1,0,0,0,
      0,Math.cos(-t/2-Math.PI),-Math.sin(-t/2-Math.PI),0,
      0,Math.sin(-t/2-Math.PI),Math.cos(-t/2-Math.PI),0,
      0,0,0,1);
  } else if (Math.cos(t/2) < 0 && Math.sin(t/2) < 0) {
    var rotMatrixX = new THREE.Matrix4().set(
      1,0,0,0,
...
// added: new matrix set for left upper and forearm
  var leftupperarmtorsoMatrix = new THREE.Matrix4().multiplyMatrices(torsoMatrix, leftupperarmMatrix);
  var rotatingleftupperarm = new THREE.Matrix4().multiplyMatrices(leftupperarmtorsoMatrix, rotMatrixX);

  var leftforearmleftupperarmMatrix = new THREE.Matrix4().multiplyMatrices(rotatingleftupperarm, leftforearmMatrix);

  leftupperarm.setMatrix(rotatingleftupperarm);
  leftforearm.setMatrix(new THREE.Matrix4().multiplyMatrices(leftforearmleftupperarmMatrix, rotMatrixX));
